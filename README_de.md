...
| Der hier beschriebene Weg hat bei mir funktioniert. Ob es bei dir auch funktioniert kann ich nicht sagen. Aber vielleicht hilft dir diese Beschreibung, den richtigen Weg für dich zu finden. Viel Glück damit. |
|:-|
...
Das Betriebssystem auf der Workstation kann entweder Linux, Windows oder iOS sein. Das ist egal. Die Workstation und auf jeden Fall der Raspberry Pi sollten via Netzwerkkabel mit dem Netzwerk verbunden sein. Der Raspberry Pi sollte nicht über WLAN in das Heimnetz integriert werden, da die Verbindung möglicherweise instabil ist.
...
Wenn der Pi zum ersten Mal mit dem neuen Betriebssystem startet, wird der Bootvorgang etwas länger dauern. Später geht das schneller. Die Fragen zu bestimmten Einstellungen (ssh- udn VNC-Aktivierung) sollten entsprechend beantwortet werden. ssh sollte auf jeden Fall aktiviert werden. An dieser Stelle ist ein angeschlossener Monitor genauso aus wie eine Tastatur mehr als nur hilfreich. Später wird der Zugriff über ssh erfolgen, aber das steht noch nicht zur Verfügung.

Sobald ssh aktiviert ist, kann über dieses Protokoll auf den Pi zugriffen werden. Alles folgende ist also auch headless via ssh möglich. Das Pi-hole-Interface wird im Webbrowser via HTML aufgerufen.
...
Nach der Installation sollte das System aktualisiert und eventuell Midnight Commander installiert werden:
...
Das Tastenkürzel `Strg-o` versetzt Midnight Commander in den Hintergrund, erneutes Drücken bringt ihn wieder in den Vordergrund. Dies ist nützlich, wenn die Konsole verwendet wird und die Ausgabe eines Befehls sichtbar sein soll, z.B. `$systemctl status hciuart.service`, den wir jetzt ausführen.
...
6) Bluetooth-Status abrufen: `$service bluetooth status`
...
4) Speichern mit `Ctrl-o`/`Strg-o`; nano mit `Ctrl-x`/`Strg-x` beenden.

_Die einfachste Erklärung ist, dass der Wert von ExecStart= gelöscht wird, sodass er überschrieben werden kann, anstatt ihn anzuhängen. Einige SystemD-Einstellungen verhalten sich wie eine angehängte Liste, wenn sie mehrmals angegeben werden. Die dritte Zeile definiert den neuen Wert von ExecStart=_
...
den Raspberry Pi suchen und auf das Stiftsymbol auf der rechten Seite klicken. Die nun geöffnete Seite zeigt die Einstellungen für den Pi. Hier die gewünschte IP-Adresse auswählen und statisch machen.
...
Nach der Installation benötigt Pi-Hole noch Listen voller bösartiger Webadressen. Solche Listen sind z.B. [hier](/../../../../RPiList/specials/blob/master/Blocklisten.md) zu finden (hauptsächlich für deutsche Nutzer, daher sind die Anleitungen auch auf Deutsch). 
...
| Schnittstelle eth0 | Schnittstellenname, „eth“ zeigt eine Kabelverbindung mit einer Ethernet-Karte an, wobei die Zahl am Ende die Nummer der Schnittstelle angibt; es sollte keine WLAN-Verbindung verwendet werden, da dies als instabil gilt|
| statische IP-Adresse=192.168.xxx.xxx/24 | <--- das muss die IP-Adresse Ihres Raspberry Pi sein, unter der der kleine Racker im Heimnetz erreichbar ist |
| Statische Router=192.168.xxx.xxx | <--- dies muss die IP-Adresse Ihres Routers sein, Standard ist 192.168.178.1 |
| statisch domain_name_servers=127.0.0.1#5335 | <--- dies ist die IP-Adresse des DNS (es muss die localhost-Adresse sein); Notieren Sie die Portadresse hinter der IP, diese ist wichtig und sollte angegeben werden, falls nicht vorhanden; es wird wahrscheinlich nicht da sein, weil noch niemand weiß, dass dieser Port benötigt wird |
...
Dieser Schritt ist notwendig, da resolvconf eine unerwünschte Datei erstellt, wenn der Eintrag aktiv ist. Nachdem der Eintrag auskommentiert wurde, sollte die unerwünschte Datei umbenannt mit
...
Diese Datei enthält Einstellungen, die [hier](https://docs.pi-hole.net/guides/dns/unbound/#configure-unbound) zu finden sind. Grundsätzlich kann der Inhalt so übernommen werden. Die einzige(!) Änderung, die ich vorgenommen habe, ist `do-ip6: no` zu `do-ip6: yes`. Aus irgendeinem Grund scheint Unbound (Version 1.9.x) IP6 verwenden zu wollen. Nach langem Suchen habe ich in den Tiefen des Internets einen Hinweis gefunden, dass dies der Grund sein könnte, warum Unbound nicht mitspielt. Nachdem ich diesen Eintrag geändert hatte, funktionierte es. Das kann daran liegen, dass dies auch in der Fritzbox standardmäßig aktiviert ist und ich es nicht abgeschaltet habe. Aber später wird es seltsam, wenn es darum geht, Pi-hole zu sagen, dass es Unbound als DNS verwenden soll.
...
Jetzt können wir Pi-hole mitteilen, dass wir ein lokales DNS haben. Dazu z.B. von der Workstation aus Pi-Hole in Ihrem Webbrowser unter der folgenden Adresse öffnen:

`http://IP-Adresse-des-Raspberrypi/admin/`
